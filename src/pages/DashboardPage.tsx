...
import { 
  ...
  ArrowRight, Landmark, Eye, Trash2, ArrowUpDown, ChevronRight, BarChart3, Receipt
} from 'lucide-react';
import { Document } from '../types';

export const DashboardPage: React.FC = () => {
  const { documents, setCurrentPath, deleteDocument, setActiveRole, user } = useWorkflow();
  ...
  const filteredDocs = documents.filter(doc => {
    const sWord = keyword.toLowerCase();
    const matchKeyword = 
  ...
  return (
    <div className="space-y-6 font-inter">
      ...
      <div className="bg-white border border-slate-200/80 rounded-2xl p-6 shadow-xs flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-full bg-[#0c4a6e]/10 border border-[#0c4a6e]/20 flex items-center justify-center font-sans font-black text-[#0c4a6e] text-lg">
            {user?.name ? user.name.split(' ').map(n => n[0]).join('') : 'DS'}
          </div>
          <div>
            <h2 className="text-xl font-extrabold text-slate-900 tracking-tight font-sans">
              Mabuhay, {user?.name || 'Officer'}!
            </h2>
            <p className="text-xs text-slate-500 mt-0.5">
              Logged in as <span className="font-semibold text-[#0c4a6e]">{user?.role || 'Budget Officer'}</span> • Workspace: {user?.email || 'dost5@workstation'}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2 bg-slate-50 hover:bg-slate-100 transition py-2 px-3 border border-slate-200 rounded-xl select-none">
          <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
          <span className="text-[11px] font-bold uppercase font-mono tracking-wider text-slate-500">
            Internal Ledger Synchronized
          </span>
        </div>
      </div>
      ...
      <div className="bg-[#0c4a6e] bg-radial from-[#0c4a6e] to-[#042f4c] text-white rounded-2xl p-6 shadow-md overflow-hidden relative">
        ...
        <div className="max-w-xl">
          <h3 className="font-sans font-black text-xl tracking-tight">Active Financial Workstation Desks</h3>
          <p className="text-xs text-slate-200 mt-1 leading-relaxed">
            Quickly jump between roles to process documents through the three mandatory state compliance workflows.
          </p>
        </div>
      ...
      <div className="space-y-4">
        <SearchBar
          ...
          onReset={() => { setKeyword(''); setSelectedType('ALL'); setSelectedSection('ALL'); }}
        />
