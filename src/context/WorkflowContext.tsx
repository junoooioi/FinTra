...
interface WorkflowContextType {
  ...
  approveBudget: (docId: string, data: { orsNumber: string; remarks: string; checklist: ChecklistItem[] }) => void;
...
export const WorkflowProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const approveBudget = (docId: string, data: { orsNumber: string; remarks: string; checklist: ChecklistItem[] }) => {
    setDocuments(prev => prev.map(doc => {
      if (doc.id === docId) {
        return {
          ...
          budgetApprovedBy: 'Maria Santos (Budget Officer)',
          budgetDateApproved: new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }),
          status: 'ACCOUNTING_PENDING',
          currentSection: 'ACCOUNTING'
        };
      }
    ...
    if (doc) {
      addAuditLog(docId, doc.title, 'Budget Approved & ORS Generated', `Obligated under ORS ${data.orsNumber}. Forwarded to Accounting section.`);
      addNotification('Forwarded to Accounting', `${docId} approved by Budget Section and routed to Chief Accountant.`, 'success', docId);
    }
  };
