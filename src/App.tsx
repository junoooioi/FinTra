...
import { LoginPage, DostLogo } from './pages/LoginPage';
import { ForgotPasswordPage } from './pages/ForgotPasswordPage';
import { AccessDeniedPage } from './components/AccessDeniedPage';
import { 
  Building2, LayoutDashboard, Receipt, BookOpen, 
  CreditCard, History, Shield, Menu, X, Clock, HelpCircle, Lock
} from 'lucide-react';
...
function ApplicationShell() {
      ...
      case 'audit':
        return (
          <div className="space-y-6">
            <div className="bg-white border border-slate-200 p-5 rounded-xl animate-fade-in">
  ...
  const navItems = [
    { id: 'dashboard', label: 'Overview Dashboard', icon: LayoutDashboard, count: 0, requiredRole: null },
    { id: 'budget', label: 'Budget Section', icon: Receipt, count: budgetCount, requiredRole: 'Budget Officer' as Role },
    { id: 'accounting', label: 'Accounting Desk', icon: BookOpen, count: acctgCount, requiredRole: 'Chief Accountant' as Role },
    { id: 'cashier', label: 'Cashier Desk', icon: CreditCard, count: cashierCount, requiredRole: 'Disbursing Cashier' as Role },
    { id: 'audit', label: 'System Audit Ledger', icon: History, count: 0, requiredRole: null },
  ];

  return (
    <div className="min-h-screen bg-tech-bg text-tech-text flex flex-col font-sans">
      
      {/* 1. Header Navigation Bar (Clean Corporate Light Theme) */}
      <header className="sticky top-0 bg-white border-b border-slate-200 h-16 flex items-center justify-between px-4 sm:px-6 z-30 shadow-xs">
        <div className="flex items-center gap-3">
          <button
            onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
            className="p-2 hover:bg-slate-100 rounded-lg lg:hidden text-slate-500 hover:text-slate-800 transition"
          >
            {mobileMenuOpen ? <X className="w-5 h-5" /> : <Menu className="w-5 h-5" />}
          </button>
          
          {/* Brand details shown on mobile header only (on desktop they live cleanly inside the sidebar) */}
          <div className="flex items-center gap-2 lg:hidden select-none">
            <DostLogo className="w-8 h-8 shrink-0" />
            <div>
              <span className="font-sans font-black text-xs tracking-tight block uppercase text-slate-800">DOST V Finance</span>
              <span className="text-[8px] text-blue-600 font-mono tracking-wider font-bold block -mt-1">ABC PORTAL</span>
            </div>
          </div>
      ...
      {user.role === 'Administrator' ? (
        <div className="md:hidden bg-slate-100 px-4 py-2 border-b border-slate-250 flex items-center justify-between text-xs gap-2 animate-fade-in">
        ...
        <div className="md:hidden bg-blue-50 border-b border-blue-100 px-4 py-2.5 flex items-center gap-2 text-xs text-slate-700 font-medium animate-fade-in select-none">
      ...
      <div className="flex-1 w-full mx-auto flex flex-col lg:flex-row relative">
        
        {/* Sidebar Nav (Desktop Left sidebar, mobile drawer) */}
        <aside className={`
          fixed lg:sticky top-0 lg:top-16 bottom-0 left-0 w-64 bg-tech-dark text-white p-0 shrink-0 z-40 transition-transform duration-300 flex flex-col justify-between border-r border-white/5
          ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}
        `}>
          <div className="flex flex-col h-full justify-between">
            <div className="flex flex-col">
              
              {/* Desktop Technical Branding Header */}
              <div className="p-6 flex items-center gap-3 border-b border-white/10 shrink-0 select-none">
                <DostLogo className="w-10 h-10 shrink-0" />
                <div>
                  <span className="font-bold tracking-tight text-sm text-white block">DOST Region V</span>
                  <span className="text-[9px] font-mono uppercase text-white/45 tracking-widest block -mt-0.5">Finance Department</span>
                </div>
              </div>

              {/* Sidebar Links */}
              <nav className="py-4 flex-1">
                <div className="px-6 py-2 text-[10px] uppercase tracking-widest text-white/40 font-black mb-2 font-mono">Main Menu</div>
                  ...
                  {navItems.map((item) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full px-6 py-3 flex items-center justify-between text-xs transition duration-150 select-none cursor-pointer ${
                          isSelected 
                            ? 'bg-white/5 border-l-4 border-tech-accent text-white font-bold' 
            ...
            <div className="p-6 border-t border-white/10 shrink-0 bg-[#0f1012]">
              <div className="flex items-center justify-between gap-2.5">
                ...
                <button
                  type="button"
                  onClick={logout}
                  title="Sign out of Google Active Space"
                  className="px-2 py-1 bg-white/5 hover:bg-red-500/10 hover:text-red-400 text-white/50 border border-white/5 hover:border-red-500/15 rounded text-[9px] font-bold tracking-wider font-mono transition duration-150 cursor-pointer shrink-0"
                >
                  DISCONNECT
                </button>
              </div>
      ...
      <footer className="bg-[#151619] text-white/40 border-t border-white/10 text-[9px] py-4 px-6 font-mono">
